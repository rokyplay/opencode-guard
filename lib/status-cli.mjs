...
function formatStatus(summary) {
  return [
    ...
    `  time window: ${valueOrDash(summary.audit.totals.first_timestamp)} -> ${valueOrDash(summary.audit.totals.latest_timestamp)}`,
...
function formatLatest(latest) {
  ...
  return `${latest.timestamp ?? "-"} ${latest.provider ?? "-"}/${latest.model ?? "-"} target=${latest.target_model ?? "-"} reviewed=${latest.reviewed_body_chars} chars segments=${latest.reviewed_segments ?? "-"}${tokens}`;
}
