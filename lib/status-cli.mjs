...
function formatLatest(latest) {
  if (!latest) return "-";
  const tokens = latest.token_usage?.exact ? ` sent_tokens=${valueOrDash(latest.token_usage.request_tokens)} received_tokens=${valueOrDash(latest.token_usage.response_tokens)} tokens_total=${valueOrDash(latest.token_usage.total_tokens)}` : "";
...
function formatTokenTotals(tokenUsage) {
  if (!tokenUsage) return "sent=0, received=0, total=0, entries=0";
  return `sent=${tokenUsage.request_tokens}, received=${tokenUsage.response_tokens}, input=${tokenUsage.input_tokens}, output=${tokenUsage.output_tokens}, total=${tokenUsage.total_tokens}, cached=${tokenUsage.cached_tokens}, reasoning=${tokenUsage.reasoning_tokens}, entries=${tokenUsage.exact_entries}`;
}
