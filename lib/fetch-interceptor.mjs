...
export function installOpenCodeModerationFetchInterceptor(options = {}) {
  ...
  target.fetch = async (input, init) => {
    ...
    const decision = await classifyRequest(request, config);
    ...
    if (!review.flagged) return await originalFetch(request);
...
async function classifyRequest(request, config) {
  ...
  if (!body || !isTargetOpenAIRequest(url, body, config)) return { review: false };
  return { review: true, body, endpointKind, context: buildReviewContext(url, body, endpointKind) };
}
...
function buildBlockedResponse(body, reason, endpointKind) {
  ...
  if (endpointKind === "responses") return buildBlockedResponsesApi(model, message);
