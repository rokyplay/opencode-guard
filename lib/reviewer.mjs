  ...
  for (const backend of config.reviewOrder) {
    if (backend === "custom") {
      if (!config.customReviewApiKey) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: config.customReviewModel || "-", status: "skipped", error: "OPENCODE_GUARD_REVIEW_API_KEY is not set in this process" }));
        continue;
      }
      if (!config.customReviewModel) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: "-", status: "skipped", error: "model is not set" }));
        continue;
      }
      try {
        return await reviewWithCustom(config.customReviewApiKey, reviewText, auditPromptText, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: config.customReviewModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error) }));
      }
      continue;
    }
    if (backend === "zen") {
      if (!config.zenApiKey) {
        errors.push(buildAttempt({ backend: "opencode-zen", model: config.zenModel, status: "skipped", error: "OPENCODEZEN_API_KEY is not set in this process" }));
        continue;
      }
      try {
        return await reviewWithZenRetries(config.zenApiKey, reviewText, auditPromptText, config.zenModel, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: "opencode-zen", model: config.zenModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error) }));
      }
      continue;
    }
    if (backend === "openai") {
      if (!config.openaiModerationApiKey) {
        errors.push(buildAttempt({ backend: "openai-moderation", model: config.openaiModerationModel, status: "skipped", error: "OPENAI_MODERATION_API_KEY is not set in this process" }));
        continue;
      }
      try {
        return await reviewWithOpenAI(config.openaiModerationApiKey, reviewText, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: "openai-moderation", model: config.openaiModerationModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error) }));
      }
      continue;
    }
    if (backend === "zen-fallbacks" && config.zenApiKey) {
      for (const model of config.zenFallbackModels) {
        try {
          ...
          errors.push(buildAttempt({ backend: "opencode-zen", model, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error) }));
        }
      }
    }
  }
  if (errors.length === 0) errors.push(buildAttempt({ backend: "none", model: "-", status: "skipped", error: "no moderation backend is configured in this process" }));
...
async function reviewWithZenRetries(apiKey, reviewText, auditPromptText, model, previousAttempts, config) {
  ...
  for (let attempt = 1; attempt <= config.zenRetries; attempt += 1) {
    try {
      ...
      lastError = error;
      previousAttempts.push(buildAttempt({ backend: "opencode-zen", model, attempt, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error) }));
...
async function reviewWithZen(apiKey, reviewText, auditPromptText, model, previousAttempts, config) {
  ...
  return { ...decision, provider: "opencode-zen", model, endpoint, http_status: response.statusCode, attempts: [...previousAttempts, buildAttempt({ backend: "opencode-zen", model, status: "ok", httpStatus: response.statusCode })], moderation_request_body_utf8_bytes: Buffer.byteLength(JSON.stringify(body), "utf8"), token_usage: buildProviderTokenUsage(response.payload.usage) };
}
...
async function reviewWithOpenAI(apiKey, reviewText, previousAttempts, config) {
  ...
  return { flagged: result.flagged, reason: result.flagged ? categories.join(", ") || "OpenAI moderation flagged" : "", provider: "openai-moderation", model, endpoint, http_status: response.statusCode, attempts: [...previousAttempts, buildAttempt({ backend: "openai-moderation", model, status: "ok", httpStatus: response.statusCode })], moderation_request_body_utf8_bytes: Buffer.byteLength(JSON.stringify(body), "utf8"), token_usage: buildProviderTokenUsage(response.payload.usage) };
}
...
function buildAttempt({ backend, model, attempt, status, error, httpStatus, http_status }) {
  return {
