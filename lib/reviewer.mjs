...
import { buildExactTokenUsage, sumExactTokenUsage, zeroExactTokenUsage } from "./token-usage.mjs";

class ModerationBackendError extends Error {
  name = "ModerationBackendError";

  constructor(attempts) {
    super("all moderation backends failed");
...
export async function reviewOutboundSegments(segments, config = getGuardConfig(), context = {}) {
  ...
  try {
    ...
    return { flagged: decision.flagged, reason: decision.flagged ? (decision.reason || "The request was flagged by the moderation guard.") : "", reviewedSegments: selected.length };
  } catch (error) {
    const failureReason = formatModerationFailureForResponse(error);
    const attempts = error instanceof ModerationBackendError ? error.attempts : [];
    const tokenUsage = attempts.length > 0 ? sumExactTokenUsage(attempts.map((attempt) => attempt.token_usage), primaryTokenEncoding(config)) : buildErrorTokenUsage(error, primaryTokenEncoding(config));
    await writeGatewayAudit(buildAuditEntry(startedAt, reviewText, { provider: "none", model: "none", flagged: true, blocked: true, reason: failureReason, error: failureReason, attempts, cache_hit: false, cache_key: null, reviewed_segments: selected.length, total_segments: segments.length, baseline_segments: uncached.length - selected.length, uncached_segments: uncached.length, cached_segments: Math.max(segments.length - uncached.length, 0), cache_write_segments: 0, token_usage: tokenUsage }, auditPromptMetadata, context), config);
    return { flagged: true, reason: formatModerationFailureForResponse(error), reviewedSegments: selected.length };
  }
}
...
async function runReviewBackends(reviewText, auditPromptText, config) {
  ...
  for (const backend of config.reviewOrder) {
    if (backend === "custom") {
      const tokenEncoding = config.customReviewTokenEncoding;
      if (!config.customReviewApiKey) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: config.customReviewModel || "-", status: "skipped", error: "OPENCODE_GUARD_REVIEW_API_KEY is not set in this process", tokenUsage: zeroExactTokenUsage(tokenEncoding) }));
        continue;
      }
      if (!config.customReviewModel) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: "-", status: "skipped", error: "model is not set", tokenUsage: zeroExactTokenUsage(tokenEncoding) }));
        continue;
      }
      try {
        return await reviewWithCustom(config.customReviewApiKey, reviewText, auditPromptText, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: `custom-${config.customReviewFormat}`, model: config.customReviewModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error), tokenUsage: buildErrorTokenUsage(error, tokenEncoding) }));
      }
      continue;
    }
    if (backend === "zen") {
      const tokenEncoding = config.zenTokenEncoding;
      if (!config.zenApiKey) {
        errors.push(buildAttempt({ backend: "opencode-zen", model: config.zenModel, status: "skipped", error: "OPENCODEZEN_API_KEY is not set in this process", tokenUsage: zeroExactTokenUsage(tokenEncoding) }));
        continue;
      }
      try {
        return await reviewWithZenRetries(config.zenApiKey, reviewText, auditPromptText, config.zenModel, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: "opencode-zen", model: config.zenModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error), tokenUsage: buildErrorTokenUsage(error, tokenEncoding) }));
      }
      continue;
    }
    if (backend === "openai") {
      const tokenEncoding = config.openaiModerationTokenEncoding;
      if (!config.openaiModerationApiKey) {
        errors.push(buildAttempt({ backend: "openai-moderation", model: config.openaiModerationModel, status: "skipped", error: "OPENAI_MODERATION_API_KEY is not set in this process", tokenUsage: zeroExactTokenUsage(tokenEncoding) }));
        continue;
      }
      try {
        return await reviewWithOpenAI(config.openaiModerationApiKey, reviewText, errors, config);
      } catch (error) {
        errors.push(buildAttempt({ backend: "openai-moderation", model: config.openaiModerationModel, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error), tokenUsage: buildErrorTokenUsage(error, tokenEncoding) }));
      }
      continue;
    }
    if (backend === "zen-fallbacks" && config.zenApiKey) {
      ...
      for (const model of config.zenFallbackModels) {
        try {
          ...
          errors.push(buildAttempt({ backend: "opencode-zen", model, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error), tokenUsage: buildErrorTokenUsage(error, tokenEncoding) }));
        }
      }
    }
  }
  if (errors.length === 0) errors.push(buildAttempt({ backend: "none", model: "-", status: "skipped", error: "no moderation backend is configured in this process", tokenUsage: zeroExactTokenUsage(primaryTokenEncoding(config)) }));
  throw new ModerationBackendError(errors);
}
...
async function reviewWithZenRetries(apiKey, reviewText, auditPromptText, model, previousAttempts, config) {
  ...
  for (let attempt = 1; attempt <= config.zenRetries; attempt += 1) {
    try {
      ...
      lastError = error;
      previousAttempts.push(buildAttempt({ backend: "opencode-zen", model, attempt, status: "failed", error: describeError(error), httpStatus: readHttpStatus(error), tokenUsage: buildErrorTokenUsage(error, config.zenTokenEncoding) }));
...
async function reviewWithZen(apiKey, reviewText, auditPromptText, model, previousAttempts, config) {
  ...
  const tokenUsage = buildExactTokenUsage({ encodingName: config.zenTokenEncoding, requestBodyText: response.requestBodyText, responseBodyText: response.responseBodyText, providerUsage: response.payload.usage });
  return { ...decision, provider: "opencode-zen", model, endpoint, http_status: response.statusCode, attempts: [...previousAttempts, buildAttempt({ backend: "opencode-zen", model, status: "ok", httpStatus: response.statusCode, tokenUsage })], moderation_request_body_utf8_bytes: Buffer.byteLength(response.requestBodyText, "utf8"), token_usage: tokenUsage };
}
...
async function reviewWithOpenAI(apiKey, reviewText, previousAttempts, config) {
  ...
  const tokenUsage = buildExactTokenUsage({ encodingName: config.openaiModerationTokenEncoding, requestBodyText: response.requestBodyText, responseBodyText: response.responseBodyText, providerUsage: response.payload.usage });
  return { flagged: result.flagged, reason: result.flagged ? categories.join(", ") || "OpenAI moderation flagged" : "", provider: "openai-moderation", model, endpoint, http_status: response.statusCode, attempts: [...previousAttempts, buildAttempt({ backend: "openai-moderation", model, status: "ok", httpStatus: response.statusCode, tokenUsage })], moderation_request_body_utf8_bytes: Buffer.byteLength(response.requestBodyText, "utf8"), token_usage: tokenUsage };
}
...
async function reviewWithCustom(apiKey, reviewText, auditPromptText, previousAttempts, config) {
  ...
  const tokenUsage = buildExactTokenUsage({ encodingName: config.customReviewTokenEncoding, requestBodyText: response.requestBodyText, responseBodyText: response.responseBodyText, providerUsage: response.payload.usage });
  return { ...decision, provider: `custom-${format}`, model, endpoint, http_status: response.statusCode, attempts: [...previousAttempts, buildAttempt({ backend: `custom-${format}`, model, status: "ok", httpStatus: response.statusCode, tokenUsage })], moderation_request_body_utf8_bytes: Buffer.byteLength(response.requestBodyText, "utf8"), token_usage: tokenUsage };
}
...
function buildErrorTokenUsage(error, encodingName) {
  return buildExactTokenUsage({
    encodingName,
    requestBodyText: readString(error?.requestBodyText) || "",
    responseBodyText: readString(error?.responseBodyText) || readString(error?.bodyText) || "",
    providerUsage: null,
  });
}

function primaryTokenEncoding(config) {
  const first = config.reviewOrder[0];
  if (first === "custom") return config.customReviewTokenEncoding;
  if (first === "zen" || first === "zen-fallbacks") return config.zenTokenEncoding;
  return config.openaiModerationTokenEncoding;
}
...
function readDiagnosticText(value) {
  return typeof value === "string" && value.length > 0 ? value.slice(0, 2000) : null;
}

function formatModerationFailureForResponse(error) {
  if (error instanceof ModerationBackendError && error.attempts.length > 0) {
    return `Moderation check failed: ${error.attempts.map(formatAttemptForResponse).join("; ")}`;
  }
  return `Moderation check failed: ${describeError(error)}`;
}

function formatAttemptForResponse(attempt) {
  const parts = [`${attempt.backend}/${attempt.model}`];
  if (attempt.attempt) parts.push(`#${attempt.attempt}`);
...
function sanitizeAttempts(attempts) {
  ...
  return attempts.map((attempt) => {
    if (typeof attempt === "string") return buildAttempt({ backend: "legacy", model: "-", status: "failed", error: attempt });
    if (!isPlainObject(attempt)) return buildAttempt({ backend: "unknown", model: "-", status: "failed", error: "invalid attempt metadata" });
    return buildAttempt(attempt);
  });
}

function buildAttempt({ backend, model, attempt, status, error, httpStatus, http_status, tokenUsage, token_usage }) {
  const exactTokenUsage = isPlainObject(tokenUsage) ? tokenUsage : (isPlainObject(token_usage) ? token_usage : zeroExactTokenUsage("o200k_base"));
  return {
    backend: sanitizeToken(backend) || "unknown",
    model: sanitizeToken(model) || "-",
    attempt: Number.isFinite(attempt) ? attempt : null,
    status: ["ok", "failed", "skipped"].includes(status) ? status : "failed",
    http_status: Number.isFinite(httpStatus) ? httpStatus : (Number.isFinite(http_status) ? http_status : null),
    error: readDiagnosticText(error),
    error_kind: classifyError(error),
    token_usage: exactTokenUsage,
  };
}
