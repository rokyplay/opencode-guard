...
export async function buildStatusSummary(config = getGuardConfig()) {
  const [prompt, audit, cache] = await Promise.all([summarizeAuditPrompt(config), summarizeAuditLog(config.auditLogFile, config.displayTimezone), summarizeHashFile(config.segmentCacheFile)]);
  return {
...
function toSafeAuditEntry(entry) {
  return {
    ...
    reason_detail: readString(entry.reason_detail),
    ...
    attempts: toSafeAttempts(entry.attempts),
...
function toSafeAttempts(attempts) {
  ...
  return attempts.map((attempt) => {
    if (!isPlainObject(attempt)) return { backend: "unknown", model: "-", attempt: null, status: "failed", http_status: null, error: "invalid attempt metadata" };
    return {
      backend: sanitizeToken(attempt.backend) || "unknown",
      model: sanitizeToken(attempt.model) || "-",
      attempt: readNullableNumber(attempt.attempt),
      status: readString(attempt.status) || "failed",
      http_status: readNullableNumber(attempt.http_status),
      error: readString(attempt.error),
      error_kind: readString(attempt.error_kind),
      token_usage: toSafeTokenUsage(attempt.token_usage),
    };
  });
}

function emptyTokenUsageTotals() {
  return { exact_entries: 0, legacy_entries_without_exact_tokens: 0, request_tokens: 0, response_tokens: 0, input_tokens: 0, output_tokens: 0, prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, cached_tokens: 0, reasoning_tokens: 0, provider_exact_entries: 0 };
}

function updateTokenUsageTotals(totals, tokenUsage) {
  if (!isPlainObject(tokenUsage) || tokenUsage.exact !== true) {
    totals.legacy_entries_without_exact_tokens += 1;
    return;
  }
  totals.exact_entries += 1;
  totals.request_tokens += readNumber(tokenUsage.request_tokens);
  totals.response_tokens += readNumber(tokenUsage.response_tokens);
  ...
  if (isPlainObject(tokenUsage.provider_usage)) totals.provider_exact_entries += 1;
}

function toSafeTokenUsage(tokenUsage) {
  if (!isPlainObject(tokenUsage) || tokenUsage.exact !== true) return null;
  return { source: readString(tokenUsage.source), exact: true, tokenizer: readString(tokenUsage.tokenizer), encoding: readString(tokenUsage.encoding), request_tokens: readNullableNumber(tokenUsage.request_tokens), response_tokens: readNullableNumber(tokenUsage.response_tokens), input_tokens: readNullableNumber(tokenUsage.input_tokens), output_tokens: readNullableNumber(tokenUsage.output_tokens), prompt_tokens: readNullableNumber(tokenUsage.prompt_tokens), completion_tokens: readNullableNumber(tokenUsage.completion_tokens), total_tokens: readNullableNumber(tokenUsage.total_tokens), cached_tokens: readNullableNumber(tokenUsage.cached_tokens), reasoning_tokens: readNullableNumber(tokenUsage.reasoning_tokens), provider_usage_present: isPlainObject(tokenUsage.provider_usage) };
}
...
function sanitizeToken(value) {
  if (typeof value !== "string") return null;
  return value.replace(/[^a-zA-Z0-9_.:-]/g, "").slice(0, 120) || null;
}
