...
async function summarizeAuditLog(filePath) {
  const file = await describeFile(filePath);
  if (!file.exists) return { file: filePath, exists: false, entries: 0, invalid_lines: 0, latest: null, totals: emptyAuditTotals() };
  ...
  for await (const line of readLines(filePath)) {
    ...
    latest = toSafeAuditEntry(entry);
  }
  totals.average_duration_ms = entries > 0 ? totals.duration_ms / entries : 0;
  return { file: filePath, exists: true, size_bytes: file.size_bytes, entries, invalid_lines: invalidLines, latest, totals };
}
